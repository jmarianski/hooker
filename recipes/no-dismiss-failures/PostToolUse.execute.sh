...
echo "$CMD" | grep -qi "test\|lint\|check\|tsc.*noEmit\|mypy\|pylint\|flake8\|eslint\|prettier.*check\|rubocop\|clippy\|go vet\|shellcheck" || return 1

# Only fire if tool had non-zero exit code
echo "$INPUT" | grep -q '"exit_code"[[:space:]]*:[[:space:]]*[1-9]' || return 1
